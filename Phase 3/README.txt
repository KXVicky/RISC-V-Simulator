...
Work Performed (Phase 3)
1. 2019CSB1115 Sahdev
    => indexAndTag() function for calculate index and tag from address 
    => BlockOffSet() function for calculate block offset from address
    => SAInst() and SAData() functions for calculating no.of accesses ,hits and misses and replacing data on miss. 
2. 2019csb1043 Ashish
   => took inputs as cachesize,blocksize,and no.of ways in SA
   => calculates number of blocks and number of sets from blocksize and cachesize
   => Data_cache() function to calculate number of bits of Block Offset ,index and tag array in address.
