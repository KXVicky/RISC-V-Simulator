    ...
    if (lines):
        for i in range(len(lines)):
            if lines[i]!='\n':
                ...
                array[0] = int(array[0],16)
                if(arr[0] >= 0x10000000):
					DataMemory[array[0]] = int(array[1], 2)
				else:
					TextMemory[array[0]] = array[1]
...
def Pipeline():
    ...
    print("IR2", IR)
    if IR!='':
        pipelineMemory_access(Operation); pipelineAlu(Operation); Operation = pipelinedecode(int(IR, 16)); IR = pipelineFetchFetch(lines)
    else:
        pipelineMemory_access(Operation); pipelineAlu(Operation); Operation = pipelinedecode(IR); IR = pipelineFetch(lines)
    for i in range(n):
        if IR!='':
            WriteBack(); pipelineMemory_access(Operation); pipelineAlu(Operation); Operation = pipelinedecode(int(IR, 16)); IR = pipelineFetch(lines)
        else:
            WriteBack(); pipelineMemory_access(Operation); pipelineAlu(Operation); Operation = pipelinedecode(IR); IR = pipelineFetch(lines)
